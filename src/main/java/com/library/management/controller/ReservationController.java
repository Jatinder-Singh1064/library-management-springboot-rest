...
//		System.out.println("60:" + user.getUserId());
		boolean isUsernameRegistered = userValidation.doesUserIdExists(user.getUserId());
//		System.out.println("62: " + isUsernameRegistered);
		...
		if (HomeController.userId.equals(""))
			...
			HomeController.errorMessage = "User is not registered !!!";
...
//		boolean isUsernameRegistered = userValidation.doesUsernameExists(user.getUsername());
	...
	/*
	 * @GetMapping("user/reserve/returnBookForm/{id}") public String
	 * userReturnBookForm(Model model, @PathVariable(name = "id") int reservationId)
	 * { Reservation reservation =
	 * reservationService.getReservationById(reservationId); int bookId =
	 * reservation.getBookId(); Book book = bookService.getBookById(bookId);
	 * 
	 * model.addAttribute("reservationId", reservationId);
	 * model.addAttribute("bookId", bookId); model.addAttribute("bookTitle",
	 * book.getTitle()); model.addAttribute("borrowDate",
	 * reservation.getBorrowDate());
	 * 
	 * if (HomeController.userId.equals("")) return "redirect:/"; else return
	 * "redirect:/admin/users/userReturnBookForm"; }
	 */

	@GetMapping("/user/reserve/returnBook")
	public String processUserReturnBook(Model model, @RequestParam(value = "bookId", required = true) int bookId) {

		if (HomeController.userId.equals(""))
			return "redirect:/";
		else {
				String message = bookService.returnBookByUser(bookId, HomeController.userId);
	...
	@GetMapping("/user/reserve/cancelReservationByUser")
	public String processCancelBookReservationByUser(Model model,
			@RequestParam(value = "id", required = true) int reservationId) {

		if (HomeController.userId.equals(""))
			return "redirect:/";
		else {
				String message = reservationService.cancelBookReservationByUser(reservationId);
				model.addAttribute("message", message);
				model.addAttribute("userId", HomeController.userId);
